...
[role="lead"]
Quando si crea un nuovo gruppo di data broker per NetApp Copy and Sync, scegliere l'opzione On-Prem Data Broker per installare il software del data broker su un host Linux locale o su un host Linux esistente nel cloud.  NetApp Copy and Sync ti guida attraverso il processo di installazione, ma i requisiti e i passaggi vengono ripetuti in questa pagina per aiutarti a preparare l'installazione.
