...
* *Compatibilità con Node.js*: v25.x
* *Sistema operativo*:
+
** CentOS 8.0 e 8.5
+
...
+
Un sistema Red Hat Enterprise Linux deve essere registrato con Red Hat Subscription Management. Se non è registrato, il sistema non può accedere ai repository per aggiornare il software di terze parti necessario durante l'installazione.
...
. Nella pagina *Definisci relazione di sincronizzazione*, scegli un'origine e una destinazione, quindi seleziona *Continua*.
+
Completa i passaggi fino a raggiungere la pagina *Data Broker Group*.

. Nella pagina *Data Broker Group*, selezionare *Create Data Broker* e quindi selezionare *On-Prem Data Broker*.
+
...
. Inserisci un nome per il data broker, quindi seleziona *Continua*.
+
Una volta caricata la pagina delle istruzioni, segui le istruzioni che includono un link univoco per scaricare il programma di installazione.

. Nella pagina delle istruzioni:
+
.. Scegli se abilitare l'accesso ad *AWS*, *Google Cloud* o entrambi.
