...
---
= Domande frequenti tecniche su NetApp Copy and Sync
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Copy Sync copia gli ACL SMB (autorizzazioni), ma non copia la proprietà di file o cartelle.  L'attributo di proprietà non è incluso nell'operazione di copia ACL SMB.  Se è necessario preservare la proprietà durante la copia dei dati tra condivisioni SMB, utilizzare `robocopy` per copiare manualmente le informazioni di sicurezza.  Ad esempio, il `/copyall` flag copia ACL, proprietario e dati di audit.
